...
Адаптер подключается к кластеру через одну или несколько **точек подключения** ([bootstrap-серверов](#bootstrap-серверы-bootstrap-servers)), указанных в справочнике **Брокеры**.
...
В адаптере параметры подключения к брокерам настраиваются в справочнике **[Брокеры](user/configuration/brokers.md)**.
...
!!! info "Порядок сообщений"
    Порядок сообщений гарантирован **внутри одной партиции**, но **не между** партициями.

### Смещение (Offset)

Порядковый номер сообщения внутри [партиции](#партиция-partition). Kafka хранит смещение для каждой [группы консьюмеров](#группа-консьюмеров-consumer-group), что позволяет точно отслеживать, до какого сообщения выполнена обработка.
...
В адаптере настройки публикации (топик, формат, сериализация) задаются в справочнике **[Продюсеры](user/configuration/producers.md)**.

### Консьюмер (Consumer)

Клиент Kafka, который **читает** сообщения из топика.

В адаптере настройки чтения (топик, формат, десериализация) задаются в справочнике **[Консьюмеры](user/configuration/consumers.md)**.
