...
**В CI** тесты встроены в конвейер выпуска: юнит- и UI-тесты запускаются параллельно в Docker-образе с полноценной 1С и PostgreSQL (виртуальный дисплей Xvfb для GUI-тестов), и файлы релиза публикуются **только после успешного прохождения** обоих контуров. Отдельным джобом считается покрытие через [Coverage41C](https://github.com/1c-syntax/Coverage41C).
