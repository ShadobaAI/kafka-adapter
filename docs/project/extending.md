...
Автоматическая регистрация выполняется через [подписки на события](modules.md#подписки-на-события). В адаптере уже определены три подписки, охватывающие **все ссылочные объекты и наборы записей регистров**.
