...
<!-- TODO: обновить скриншот admin-queues.png — на форме «Очереди» появились блок «Контроль интеграции» и поле «Идемпотентность» (срок хранения хэш-сумм) -->
...
Отдельное поле **Идемпотентность** задаёт срок хранения (в днях, по умолчанию **3**) записей регистра «Хэш-суммы сообщений» — истории хэшей для [идемпотентной отправки и обработки](producers.md#идемпотентная-отправка). После удаления хэша по сроку хранения сообщение с неизменным содержимым снова уйдёт в Kafka / будет обработано.

Удаление по срокам хранения выполняет регламентное задание. Ручная очистка очередей — [Обслуживание очередей](../operations/queue-maintenance.md).
