...
## Нужно очистить локальную очередь

При тестировании и отладке накопившиеся записи очередей удаляются командами группы **Удалить сообщения** на формах списков «Исходящие / Входящие сообщения» — полностью, по отбору (в т.ч. по продюсеру/консьюмеру) или выделенными записями. Требуются права администратора адаптера и остановленный обмен. Подробности — [Обслуживание очередей](queue-maintenance.md).
