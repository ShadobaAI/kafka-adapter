...
| Слой | Основные объекты | Ответственность |
|------|------------------|-----------------|
| Публичный фасад | `кфкИнтеграция`, `кфкИнтеграцияКлиент` | Стабильные точки входа прикладного кода |
| Регистрация | Подписки на события, `кфкОбработкаСобытийСлужебный` | Отбор изменений и постановка в очередь |
| Хранение состояния | Очереди и служебные регистры | Устойчивое состояние сообщений и фоновых операций |
| Оркестрация | `кфкФоновыеОперацииСлужебный` | Диспетчеризация, параллельная обработка, очистка и контроль |
| Преобразование | Прикладные обработчики, `кфкОбменДаннымиXDTOСервер` | Сериализация и десериализация бизнес-данных |
| Транспорт | Обработка `кфкИнтеграция` | Сессии внешней компоненты и операции Kafka |
| Администрирование | `кфкПанельАдминистрирования` | Настройка, очереди и диагностика |

Прикладное решение отвечает за бизнес-события, контракты сообщений, обработчики и идемпотентность бизнес-операций. Адаптер отвечает за очереди, транспорт, повторы, диспетчеризацию и наблюдаемое техническое состояние. Apache Kafka и эксплуатация кластера находятся за границей подсистемы.

Изменение сигнатур публичного фасада, схемы или семантики очередей, распределения фоновых потоков, поведения внешней компоненты либо contract обработчиков считается изменением архитектурной границы. Для таких изменений нужны SDD, compatibility analysis, документация и релевантные тесты; устойчивое изменение границ компонентов дополнительно требует ADR.
