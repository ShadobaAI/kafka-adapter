...
flowchart LR
    subgraph Adapter["Адаптер Kafka"]
        ...
        subgraph WORKERS["Фоновые задания"]
            ...
            WORKERN[[Поток N]]
            PROCESS["..."]
            HASH{{"Идемпотентная отправка:<br/>хэш тела изменился?"}}
            CANCEL["Статус «Выгрузка отменена»"]
    ...
    PROCESS --> HASH
    HASH -->|"да / выключена"| DLL -->|отправка| TOPIC
    HASH -->|нет| CANCEL
    DLL --> PROCESS
...
- при включённой у продюсера [идемпотентной отправке](../user/configuration/producers.md#идемпотентная-отправка) сравнивают SHA-256-хэш тела сообщения с хэшем предыдущей выгрузки той же записи очереди (регистр «Хэш-суммы сообщений»): если содержимое не изменилось — запись получает статус **«Выгрузка отменена»** («Сообщение уже выгружено») и в Kafka не отправляется;
- выполняют отправку тела сообщения в Kafka через внешний компонент;
- обновляют состояние сообщения по результатам отправки (при успешной выгрузке обновляется и сохранённый хэш).
...
```mermaid
flowchart LR
    QUEUE[\"РС Входящие сообщения"/] --> DISPATCH[[Диспетчер ФЗ]]
    DISPATCH --> WORKERS[[Потоки десериализации]]
    WORKERS --> HASH{{"Идемпотентная обработка:<br/>хэш тела изменился?"}}
    HASH -->|"да / выключена"| HANDLER["Обработчик / КД 3.1"]
    HASH -->|нет| CANCEL["Статус «Обработка отменена»"]
    HANDLER -->|обновление статуса| QUEUE
    CANCEL --> QUEUE
```

Потоки десериализации:

- при включённой у консьюмера [идемпотентной обработке](../user/configuration/consumers.md#идемпотентная-обработка) сравнивают SHA-256-хэш тела сообщения с хэшем предыдущей обработки той же записи очереди: если содержимое не изменилось — запись получает статус **«Обработка отменена»** («Сообщение уже обработано»), обработчик не вызывается;
- преобразуют тело сообщения в формат, ожидаемый прикладным решением;
- выполняют прикладную обработку сообщений;
- обновляют состояние сообщений по результатам обработки (при успешной обработке обновляется и сохранённый хэш).
...
## Границы отказов и восстановление

| Отказ или решение | Где сохраняются данные | Кто отвечает за восстановление |
|--------------------|------------------------|--------------------------------|
| Ошибка автоматической регистрации | Журнал регистрации 1С | Разработчик адаптера или оператор |
| Ошибка сериализации/десериализации | Статус и журнал записи очереди | Владелец прикладного обработчика |
| Ошибка отправки в Kafka | Статус исходящего сообщения и счётчик попыток | Сначала автоматический повтор, затем оператор |
| Дубль или неизменившееся тело | Статус отмены/дубля | Ожидаемое решение адаптера |
| Прерывание фонового задания | Очередь сообщений и служебные позиции операций | Диспетчер и оператор |

Устойчивое состояние очередей позволяет продолжить обработку после перезапуска, но не создаёт гарантию exactly-once. Прикладные обработчики должны использовать стабильные ключи, идемпотентные эффекты и наблюдаемые переходы статусов.
