...
// Возвращает Истина для ссылочного типа; для типа Неопределено возвращает Ложь.
//
// Замеры:
//	* ОбщегоНазначения.ЭтоСсылка - Длительность = 0.014с, Среднее = 0.000014с
//	* кфкОбщегоНазначенияСлужебный.ЭтоСсылка - Длительность = 0.008с, Среднее = 0.000008с.
//	
// Параметры:
...
// Возвращает Истина, если переданное имя объекта метаданных является ссылочным типом
// 
// Параметры:
//	ИмяОбъекта - Строка - Полное имя объекта метаданных, например "Справочник.Номенклатура"
// 
// Возвращаемое значение:
//	Булево - Ссылочный тип
Функция ЭтоИмяСсылочногоТипа(ИмяОбъекта) Экспорт
	
	// BSLLS:Typo-off
	Возврат СтрНачинаетсяС(ИмяОбъекта, "справочник.")
		...
		Или СтрНачинаетсяС(ИмяОбъекта, "задача.");
	// BSLLS:Typo-on
...
// Приостанавливает выполнение кода на заданный интервал.
//
// Параметры:
...
Функция ЗначениеВJSON(Знач Значение) Экспорт
	...
	ЗаписатьJSON(ЗаписьJSON, Значение, , "ПреобразованиеЗначенияВJSON", кфкОбщегоНазначенияСлужебный); 
	
	Возврат ЗаписьJSON.Закрыть();
	
КонецФункции

// Функция вызывается для всех свойств, тип которых не поддерживает преобразование в формат JSON напрямую, см.
//    ЗначениеВJSON
// 
// Результат: Значение.
//
...
Функция ПрочитатьGZip(СжатыеДанные) Экспорт
	...
	ПотокZip = Новый ПотокВПамяти(ZipLFHSize() + РазмерСжатыхДанных + ZipDDSize() + ZipCDHSize() + ZipEOCDSize());
...
// Возвращает дату из миллисекунд
// 
// Параметры:
//	Миллисекунды - Число - Миллисекунды
// 
// Возвращаемое значение:
//	Дата - Дата из миллисекунд
Функция ДатаИзМС(Миллисекунды) Экспорт
	
	Результат = Дата(1, 1, 1);
	
	Если Миллисекунды > 0 Тогда
		Результат = Результат + Миллисекунды / 1000	+ СмещениеСтандартногоВремени(ЧасовойПоясСеанса());
	КонецЕсли;
...
// Проверяет, доступна ли подсистема для текущего пользователя
// 
// Возвращаемое значение:
//  Булево - Подсистема доступна
...
#Иначе
	...
	ЧтениеZip = Новый ЧтениеФайлаАрхива(СжатыеДанные);
	ИмяФайла = ЧтениеZip.Элементы[0].Имя;
	Попытка
		ЧтениеZip.Извлечь(ЧтениеZip.Элементы[0], Каталог, РежимВосстановленияПутейФайлаАрхива.НеВосстанавливать);
	Исключение
	...
	УдалитьФайлы(Каталог); // BSLLS:UsingSynchronousCalls-off
...
#Иначе
	...
	ЗаписьZip = Новый ЗаписьФайлаАрхива(ПотокZip);
	ЗаписьZip.Добавить(ВременныйФайл);
	ЗаписьZip.Записать();
	УдалитьФайлы(ВременныйФайл); // BSLLS:UsingSynchronousCalls-off
...
// Значения и смещения определены форматом ZIP.
// BSLLS:MagicNumber-off
...
Функция ZipLFH()
	...
	// Local file header
	Буфер = Новый БуферДвоичныхДанных(ZipLFHSize());
...
Функция ZipDD(CRC32, РазмерСжатыхДанных, РазмерНесжатыхДанных)
	...
	// Data descriptor
	Буфер = Новый БуферДвоичныхДанных(ZipDDSize());
...
Функция ZipCDH(CRC32, РазмерСжатыхДанных, РазмерНесжатыхДанных)
	...
	Буфер = Новый БуферДвоичныхДанных(ZipCDHSize());
...
Функция ZipEOCD(РазмерСжатыхДанных)
	...
	Буфер = Новый БуферДвоичныхДанных(ZipEOCDSize());
	...
	Буфер.ЗаписатьЦелое32(12, ZipCDHSize()); // size of the central directory
	// offset of start of central directory with respect to the starting disk number
	Буфер.ЗаписатьЦелое32(16, ZipLFHSize() + РазмерСжатыхДанных + ZipDDSize());
...
// Значения и смещения определены RFC 1952.
// BSLLS:MagicNumber-off

// Описание структур см. здесь https://www.ietf.org/rfc/rfc1952.txt

Функция GZipHeaderSize()
	Возврат 10;
КонецФункции

Функция GZipFooterSize()
	Возврат 8;
КонецФункции

Функция GZipHeader()

	Буфер = Новый БуферДвоичныхДанных(GZipHeaderSize());
	Буфер[0] = 31;                    // ID1 0x1f
...
Функция GZipFooter(CRC32, РазмерИсходныхДанных)
	...
	Буфер = Новый БуферДвоичныхДанных(GZipFooterSize());
