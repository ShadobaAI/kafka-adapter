...
Функция ТаблицаПравилОбработкиДанных(КомпонентыОбмена) Экспорт
	...
	КомпонентыОбмена.МенеджерОбмена.ЗаполнитьПравилаОбработкиДанных(КомпонентыОбмена.НаправлениеОбмена,
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если КомпонентыОбмена.НаправлениеОбмена = НаправлениеОбменаПолучение() Тогда
			...
			Если КомпонентыОбмена.ЭтоEnterpriseData Тогда
				...
				Если КлючевыеСвойства <> Неопределено Тогда
					...
					СвойствоСсылкаXDTO = ТипКлючевыхСвойствОбъектаXDTO.Свойства
		...
		Иначе
			// Для правила без объекта метаданных имя таблицы выборки не заполняется.
			Продолжить;
		КонецЕсли;
...
Функция ИнициализироватьТаблицуСвойствДляПравилаКонвертации() Экспорт
	...
	ТипСтрока100 = ОбщегоНазначения.ОписаниеТипаСтрока(100); // BSLLS:MagicNumber-off
	...
	ТаблицаПКС.Колонки.Добавить("ПравилоКонвертацииСвойства", ТипСтрока100);
...
Функция КоллекцияПравилКонвертации()
	...
	ТипБулево = Новый ОписаниеТипов("Булево");
	ТипСтрока0 = ОбщегоНазначения.ОписаниеТипаСтрока(0);
	ТипСтрока60 = ОбщегоНазначения.ОписаниеТипаСтрока(60);
	...
	ПравилаКонвертации.Колонки.Добавить("ТипПолученныхДанныхСтрокой", ТипСтрока300);
	ПравилаКонвертации.Колонки.Добавить("ИмяТаблицыПолученныхДанных", ТипСтрока300);
	ПравилаКонвертации.Колонки.Добавить("ПредставлениеТипаПолученныхДанных", ТипСтрока300);
	...
	ПравилаКонвертации.Колонки.Добавить("ПоляПредставленияОбъекта", ТипСтрока300);
	...
	ПравилаКонвертации.Колонки.Добавить("ВариантИдентификации", ТипСтрока60);
...
Функция ТаблицаПравилКонвертации(КомпонентыОбмена)
	...
	КомпонентыОбмена.МенеджерОбмена.ЗаполнитьПравилаКонвертацииОбъектов(КомпонентыОбмена.НаправлениеОбмена,
		ПравилаКонвертации);
	...
	Если КомпонентыОбмена.НаправлениеОбмена = НаправлениеОбменаПолучение() Тогда
		...
		Для Каждого СтрокаОбработки Из СтрокиДляОбработки Цикл
			...
			СтрокаПравилОбработкиДанных = КомпонентыОбмена.ПравилаОбработкиДанных
				.Найти(СтрокаОбработки.ОбъектФормата, "ОбъектВыборкиФормат");
			Если СтрокаПравилОбработкиДанных <> Неопределено Тогда
				МассивИспользуемыхПКО = СтрокаПравилОбработкиДанных.ИспользуемыеПКО;
				СтрокаОбработки.РазрешитьСоздаватьОбъектИзСтруктуры = МассивИспользуемыхПКО
					.Найти(СтрокаОбработки.ИмяПКО) = Неопределено;
			КонецЕсли;
		КонецЦикла;
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если ЗначениеЗаполнено(ПравилоКонвертации.ОбъектФормата) Тогда
			...
			Если ПравилоКонвертации.ТипXDTO = Неопределено Тогда
				КомпонентыОбмена.ОбъектыФормата.Вставить(ПравилоКонвертации.ИмяПКО, ПравилоКонвертации.ОбъектФормата);
		...
		Если КомпонентыОбмена.НаправлениеОбмена = НаправлениеОбменаПолучение() Тогда
			...
			ПравилоКонвертации.ПоляПредставленияОбъекта = ?(ПравилоКонвертации.ПоляПоиска.Количество() = 0,
		...
		Если ПравилоКонвертации.ОбъектДанных <> Неопределено Тогда
			...
			Иначе
				// Для неподдерживаемого объекта метаданных признаки типа остаются сброшенными.
				ПравилоКонвертации.ЭтоКонстанта = Ложь;
			КонецЕсли;
		...
		Если ПравилоКонвертации.ВариантИдентификации = "ПоПолямПоиска"
			Или ПравилоКонвертации.ВариантИдентификации = ВариантИдентификацииСначалаПоУИДПотомПоПолям() Тогда
...
Функция ДанныеФорматаИзДанныхИБ(КомпонентыОбмена, Знач ПравилоКонвертации, Источник, СтекВыгрузки = Неопределено)
	...
	Если ПравилоКонвертации.ЭтоКонстанта Тогда
		...
		Иначе
			ШаблонОшибки = НСтр("ru = 'Ошибка XML-схемы. Для приемника должно быть задано одно свойство.
				|Тип источника: %1
				|Тип приемника: %2'");
			ТипИсточникаСтрокой = Строка(ТипЗнч(Источник));
			ТекстОшибки = СтрШаблон(ШаблонОшибки, ТипИсточникаСтрокой, ПравилоКонвертации.ТипXDTO);
...
Процедура ВыгрузитьСвойстваВторогоЭтапа(
	...
	Для Каждого ПКС Из ПравилоКонвертации.Свойства Цикл
		...
		Если ПКС.ИспользуетсяАлгоритмКонвертации Тогда
			...
			Если ЗначениеЯвляетсяИнструкцией Или ЕстьПравилоДляОбычногоЗначения Тогда
...
Процедура ВыгрузитьСвойство(КомпонентыОбмена, ДанныеИБ, ПолучательСвойства, ПКС, СтекВыгрузки, ЭтапВыгрузки = 1)
	...
	Если ЭтапВыгрузки = 1 Тогда
		...
		ИначеЕсли ТипЗнч(ДанныеИБ) = ТипСтруктура() Тогда
			...
			КонецЕсли;
		Иначе
			// Для источника другого типа значение свойства остается неопределенным.
			ЗначениеСвойства = Неопределено;
		КонецЕсли;
	Иначе
		...
		Если ТипЗнч(ПолучательСвойства) = Тип("СтрокаТаблицыЗначений") Тогда
			...
			Если УказаноПолноеИмяСвойства Тогда
				Для Уровень = 0 По УровеньМаксимальный Цикл
					...
					ИначеЕсли ТипЗнч(ЗначениеВКолонке) = ТипСтруктура() Тогда
						...
						Для УровеньПодчиненный = Уровень + 1 По УровеньМаксимальный Цикл
							...
							Если НЕ ИсточникВложенногоСвойства.Свойство(ИмяВложенногоСвойства,
						...
						КонецЦикла;
					Иначе
						// Значение неподдерживаемого вложенного типа пропускается.
						Продолжить;
					КонецЕсли;
				КонецЦикла;
			Иначе
		...
		Иначе
			...
			КонецЕсли;
			Если КомпонентыОбмена.ЭтоEnterpriseData И ЗначениеСвойства = Неопределено Тогда
				КлючевоеСвойствоНайдено = СтекВыгрузки.Количество() = 1
					И ПолучательСвойства[ОбменДаннымиXDTOСервер.КлассКлючевыеСвойства()]
						.Свойство(СвойствоФормата_Имя, ЗначениеСвойства);
				Если Не КлючевоеСвойствоНайдено Тогда
					Возврат;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если СтекВыгрузки.Количество() = 1 И ПКС.ОбработкаКлючевогоСвойства Тогда
		...
		КонецЕсли;
		ПолучательСвойства[ОбменДаннымиXDTOСервер.КлассКлючевыеСвойства()]
			.Вставить(СвойствоФормата_Имя, ЗначениеСвойства);
	Иначе
		Если ТипЗнч(ПолучательСвойства) = Тип("СтрокаТаблицыЗначений") Тогда
			Если УказаноПолноеИмяСвойства Тогда
...
// 
Функция ОбъектФорматаИзДанныхФормата(КомпонентыОбмена, Знач Источник, Знач ТипXDTO, ПроведеноЗаполнениеСвойств = Ложь)
	
	Приемник = ФабрикаXDTO.Создать(ТипXDTO);
	
	СвойстваИсточника = СвойстваТипаФорматаМассивом(ТипXDTO);
	...
	Для Каждого Свойство Из СвойстваИсточника Цикл
		...
		ЗначениеСвойства = Неопределено;
		СвойствоНайдено = ПолучитьЗначениеСвойстваИсточникаФормата(Источник, Свойство.Имя, ЗначениеСвойства);
		...
		Попытка
			Если ТипСвойства = ТипОбщегоСоставногоСвойства() Тогда
				...
				Если ТипЗнч(Источник) = ТипСтруктура() И СвойствоНайдено Тогда
					...
					ЗначениеXDTO = ОбъектФорматаИзДанныхФормата(КомпонентыОбмена, ЗначениеСвойства,
						Свойство.Тип, ПроведеноЗаполнениеВложенныхСвойств);
						
				Иначе
					
					ЗначениеXDTO = ОбъектФорматаИзДанныхФормата(КомпонентыОбмена, Источник,
						Свойство.Тип, ПроведеноЗаполнениеВложенныхСвойств);
			...
			Иначе
				...
				Если Не СвойствоНайдено Или ЗначениеНеЗаполнено(КомпонентыОбмена, ЗначениеСвойства) Тогда
					Продолжить;
				КонецЕсли;
				...
				Если ТипСвойства = ОбменДаннымиXDTOСервер.КлассКлючевыеСвойства() Тогда
					...
					ЗначениеXDTO = ОбъектФорматаИзДанныхФормата(КомпонентыОбмена, ЗначениеСвойства, Свойство.Тип);
				...
				ИначеЕсли ТипСвойства = "Таблица" Тогда
					...
					Для Каждого ИсточникСтрока Из ЗначениеСвойства Цикл
						...
						ПриемникСтрока = ОбъектФорматаИзДанныхФормата(КомпонентыОбмена, ИсточникСтрока, ТипТаблицы, Ложь);
				...
				ИначеЕсли ТипСвойства = "МассивИзОбычноеСвойство" Тогда
					...
					Для Каждого Элемент Из ЗначениеСвойства Цикл
						...
						Если ЗначениеНеЗаполнено(КомпонентыОбмена, Элемент) Тогда
							ЭлементXDTO = Неопределено;
							
						ИначеЕсли ЭтоЭлементСсылки Тогда // Конвертация ссылки
							ЭлементXDTO = ЗначениеВСтроку(Элемент);
							
						ИначеЕсли ЭтоЭлементПеречисления Тогда // Конвертация перечисления
							ЭлементXDTO = КонвертироватьПеречислениеВXDTO(КомпонентыОбмена, Элемент,
								СвойствоТипЗначения);
							
						ИначеЕсли кфкОбщегоНазначенияСлужебный.ЭтоСсылка(Элемент) Тогда // Конвертация ссылки в строку
				...
				ИначеЕсли ТипСвойства = "СвойствоСоставногоТипа" Тогда
					...
					Для Каждого СвойствоСоставногоТипа Из Свойство.Тип.Свойства Цикл
						...
						Если ТипЗнч(ЗначениеСвойства) = ТипСтруктура()
							...
							СоставноеЗначениеXDTO = ОбъектФорматаИзДанныхФормата(КомпонентыОбмена, ЗначениеСвойства,
								СвойствоСоставногоТипа.Тип);
							
						ИначеЕсли ЗначениеСоответствуетПростомуТипуФормата(
								ЗначениеСвойства, СвойствоСоставногоТипа.Тип) Тогда
						...
						ИначеЕсли ТипЗнч(ЗначениеСвойства) = Тип("Строка")
							...
							КонецЕсли;
						Иначе
							// Значение, не соответствующее вариантам составного типа, пропускается.
							Продолжить;
						КонецЕсли;
				...
				Иначе
					// Для неподдерживаемого класса свойства значение XDTO не формируется.
					ЗначениеXDTO = Неопределено;
				КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(ШаблонОшибки, ТипСвойства, Свойство.Имя);
			ОбработатьИсключение(ИнформацияОбИсключении, ТекстОшибки);
		КонецПопытки;
...
Процедура ДополнитьОбъектXDTO(ОбъектXDTO, Знач Свойство, Знач Значение)
	...
	ИначеЕсли ТипЗнч(Свойство.Тип) = Тип("ТипОбъектаXDTO") Тогда
		Для Каждого ДочернееСвойство Из Значение.Свойства() Цикл
			...
			ЭлементЗначения = Значение.Получить(ДочернееСвойство.Имя);
			...
			Если ТипЗнч(ДочернееСвойство.Тип) = Тип("ТипЗначенияXDTO") Тогда
				ДополнитьОбъектXDTO(ОбъектXDTO[Свойство.Имя], ДочернееСвойство, 
					ФабрикаXDTO.Создать(ДочернееСвойство.Тип, ЭлементЗначения));
			Иначе
				ДополнитьОбъектXDTO(ОбъектXDTO[Свойство.Имя], ДочернееСвойство, ЭлементЗначения);
			КонецЕсли;
		КонецЦикла;
	Иначе
		// Свойства других типов не требуют дополнения объекта XDTO.
		Возврат;
	КонецЕсли;
			...
			ИначеЕсли ЭтоСписокXDTO Тогда
				...
				Для Каждого ЭлементСпискаXDTO Из ЗначениеСвойстваXDTO Цикл
					...
					Если ТипЗнч(Свойство.Тип) = Тип("ТипОбъектаXDTO") Тогда
						ЗаполнитьОшибкиПроверкиОбъектаXDTO(ЭлементСпискаXDTO, Свойство.Тип,
							НовыйСтекОшибок, Уровень + 1);
					Иначе
						Попытка
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ЗначениеXDTO) = Тип("ОбъектXDTO") Тогда
			...
			ИначеЕсли КлассЗначенияXDTO = КлассТаблицаОбъекта() Тогда
				...
				КонвертироватьТаблицуОбъекта(Приемник, КомпонентыОбмена, ИмяСвойства, Свойство, ТипЗначенияXDTO,
					ЗначениеXDTO);
			...
			КонецЕсли;
		Иначе
			// Значения других типов не преобразуются в элемент структуры.
			Возврат;
		КонецЕсли;
	Исключение
		ОбработатьИсключение(ИнформацияОбОшибке(),
...
Процедура КонвертироватьОбщееСвойство(Приемник, Знач КомпонентыОбмена, Знач ИмяСвойства,
	...
	Если ТипЗнч(Приемник) = ТипСтруктура() Тогда
		...
		Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
			...
			КонвертацияСвойстваФорматаВЭлементСтруктуры(ЗначениеXDTO, ПодСвойство, ПриемникГруппыСвойств,
				КомпонентыОбмена, , ОписаниеСвойства.Тип);
		...
		Для Каждого СвойствоГруппы Из ПриемникГруппыСвойств Цикл
			...
			Если НЕ Приемник.Свойство(ИмяПодСвойства)
				И НЕ (ЕстьКлючевыеСвойства
						И Приемник[ОбменДаннымиXDTOСервер.КлассКлючевыеСвойства()].Свойство(ИмяПодСвойства)) Тогда
	...
	Иначе
		...
		Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
			...
			КонвертацияСвойстваФорматаВЭлементСтруктуры(ЗначениеXDTO, ПодСвойство, Приемник, КомпонентыОбмена, ,
...
Функция СтруктураОбъектаФорматаВДанныеИБ(КомпонентыОбмена, Знач ПравилоКонвертации, ДанныеXDTO,
	...
	ЗначениеДействияПолучитьСсылку = ДействиеПолучитьСсылку();
	ЗначениеДействияКонвертироватьИЗаписать = ДействиеКонвертироватьИЗаписать();
	ЗначениеВариантаСначалаПоУИДПотомПоПолям = ВариантИдентификацииСначалаПоУИДПотомПоПолям();

	ДанныеИБ = Неопределено;
	ПолученныеДанные = ИнициализироватьПолученныеДанные(ПравилоКонвертации);
	
	СоставСвойств = ?(Действие = ЗначениеДействияПолучитьСсылку, "СвойстваПоиска", "Все");
	ПолученныеДанныеСсылка = Неопределено;
	
	КлассСсылкаФормата = КлассСсылкаФормата(КомпонентыОбмена, ПравилоКонвертации.Свойства);
	ДанныеФорматаСодержатСсылку = ДанныеXDTO.Свойство(КлассСсылкаФормата);
	...
	ВариантИдентификацииПоИдентификатору = ВариантИдентификации = "ПоУникальномуИдентификатору"
		Или ВариантИдентификации = ЗначениеВариантаСначалаПоУИДПотомПоПолям;
	
	// Получение ссылки на объект ИБ.
	Если ПравилоКонвертации.ЭтоСсылочныйТип Тогда
		
		ПолученныеДанныеСсылка = ПолученныеДанные.Ссылка;
		Если ДанныеФорматаСодержатСсылку И ВариантИдентификацииПоИдентификатору Тогда
			...
			Если Действие = ЗначениеДействияПолучитьСсылку Тогда
	...
	Если Действие = ЗначениеДействияПолучитьСсылку И КомпонентыОбмена.ЭтоEnterpriseData Тогда
	...
	ИдентифицироватьПоПолям = ВариантИдентификации = "ПоПолямПоиска"
		Или ВариантИдентификации = ЗначениеВариантаСначалаПоУИДПотомПоПолям;
	Если ДанныеИБ = Неопределено И Не ПравилоКонвертации.ЭтоРегистр И ИдентифицироватьПоПолям Тогда
		
		ДанныеИБ = СсылкаОбъектаПоСвойствамОбъектаФормата(
			ПравилоКонвертации,
			ПолученныеДанные,
	...
	Если ВариантИдентификации = "ПоПолямПоиска"
			Или ВариантИдентификации = ЗначениеВариантаСначалаПоУИДПотомПоПолям Тогда
	...
	Если ЗаписыватьОбъектВИБ Тогда
		Возврат ЗаписатьПолученныеДанныеВИБ(
			...
			ДанныеИБ,
			Действие,
			ЗначениеДействияПолучитьСсылку,
...
Функция ЗаписатьПолученныеДанныеВИБ(КомпонентыОбмена, ПравилоКонвертации, ПолученныеДанные, ДанныеИБ,
	...
	Попытка
		...
		Для Каждого ТЧ Из ПравилоКонвертации.СвойстваТабличныхЧастей Цикл
			...
			Если НомерЭтапа = 1 И ЗначениеЗаполнено(ТЧ.ТЧКонфигурации) И ЗначениеЗаполнено(ТЧ.ТЧФормата) Тогда
				...
				Если НЕ ДанныеXDTO.Свойство(ТЧ.ТЧФормата, ТЧФормата) Или ТЧФормата.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;
			...
			ОбрабатыватьСтрокиТабличнойЧасти = НомерЭтапа = НомерВторогоЭтапаКонвертации()
				И ТЧ.ИспользуетсяАлгоритмКонвертации
				И ЗначениеЗаполнено(ТЧ.ТЧКонфигурации)
				И ПолученныеДанные.ДополнительныеСвойства.Свойство(ТЧ.ТЧКонфигурации);
			Если ОбрабатыватьСтрокиТабличнойЧасти Тогда
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Попытка
			...
			Иначе
				...
				Если НЕ ИсточникДанных.Свойство(СвойствоФормата_Имя, ЗначениеСвойства)
					...
					Если ПриемникДанных.ДополнительныеСвойства.Свойство("СвойстваОтсутствующиеВПолученныхДанных") Тогда
						...
						ПриемникДанных.ДополнительныеСвойства.СвойстваОтсутствующиеВПолученныхДанных
							= ПриемникДанных.ДополнительныеСвойства.СвойстваОтсутствующиеВПолученныхДанных
								+ "," + СвойствоФормата_Имя;
						
					Иначе
						
						ПриемникДанных.ДополнительныеСвойства.Вставить("СвойстваОтсутствующиеВПолученныхДанных",
							СвойствоФормата_Имя);
		...
		ИначеЕсли НомерЭтапа = НомерВторогоЭтапаКонвертации() Тогда
			...
			КонецЕсли;
		Иначе
			// На других этапах значение свойства не конвертируется.
			Возврат;
		КонецЕсли;
		...
		ДействиеПолученияСсылки = ДействиеПолучитьСсылку();
		ДанныеДляЗаписиВИБ = СтруктураОбъектаФорматаВДанныеИБ(КомпонентыОбмена, ПравилоКонвертации, ЗначениеСвойства,
			ДействиеПолученияСсылки);
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(ШаблонОшибки, ПКС.СвойствоКонфигурации);
		ОбработатьИсключение(ИнформацияОбИсключении, ТекстОшибки);
	КонецПопытки;
	
КонецПроцедуры

Функция СсылкаОбъектаПоСвойствамОбъектаФормата(ПравилоКонвертации, ПолученныеДанные, ДанныеФорматаСодержатСсылку,
	...
	Для Каждого ПопыткаПоиска Из ПравилоКонвертации.ПоляПоиска Цикл
		...
		Если ПоискЗапросом Тогда
			...
			Для Каждого ПолеПоиска Из ПоляПоиска Цикл
				...
				Если ОбменДаннымиПовтИсп.ЭтоСтроковыйРеквизитНеограниченнойДлины(ПравилоКонвертации.ПолноеИмя,
					...
					ОтборСтрокой = СтрЗаменить(ОтборСтрокой, "[ДлинаСтроки]",
