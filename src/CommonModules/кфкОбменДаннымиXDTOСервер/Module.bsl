...
Функция ИнициализироватьТаблицуСвойствДляПравилаКонвертации() Экспорт
	...
	ТаблицаПКС.Колонки.Добавить("СвойствоКонфигурации", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаПКС.Колонки.Добавить("СвойствоФормата", ОбщегоНазначения.ОписаниеТипаСтрока(0));
...
Функция КоллекцияПравилКонвертации()
	...
	ПравилаКонвертации.Колонки.Добавить("ОбъектФормата",  ОбщегоНазначения.ОписаниеТипаСтрока(0));
	...
	ПравилаКонвертации.Колонки.Добавить("ПространствоИмен",  ОбщегоНазначения.ОписаниеТипаСтрока(0));
...
Функция ПредставлениеОшибки(ИнформацияОбОшибке)
	...
	ИначеЕсли Заголовки.Количество() Тогда
		Результат = Результат + Символы.ПС
			+ Символы.ПС
			+ СтрШаблон(НСтр("ru = 'Дополнительная информация:
			|	%1'"), СтрСоединить(Заголовки, Символы.ПС + Символы.Таб)); 
	...
	Иначе
		...
		Иначе
			...
			Если КомпонентыОбмена.ЭтоEnterpriseData И ЗначениеСвойства = Неопределено
				И Не (СтекВыгрузки.Количество() = 1
				И ПолучательСвойства[ОбменДаннымиXDTOСервер.КлассКлючевыеСвойства()]
					.Свойство(СвойствоФормата_Имя, ЗначениеСвойства)) Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
			...
			Иначе
				...
				ИначеЕсли ТипСвойства = "МассивИзОбычноеСвойство" Тогда
					...
					Для Каждого Элемент Из ЗначениеСвойства Цикл
						...
						Если Элемент = Null Или Элемент = Неопределено
							Или Не (КомпонентыОбмена.ОтключитьПреобразованиеВNull Или ЗначениеЗаполнено(Элемент)) Тогда
				...
				ИначеЕсли ТипСвойства = "СвойствоСоставногоТипа" Тогда
					...
					Для Каждого СвойствоСоставногоТипа Из Свойство.Тип.Свойства Цикл
						...
						Если ТипЗнч(ЗначениеСвойства) = Тип("Структура")
							...
							СоставноеЗначениеXDTO = ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, ЗначениеСвойства,
...
Процедура ДобавитьОшибкуПроверки(СтекОшибок, Уровень, Свойство, ШаблонОшибки)
	...
	СтекОшибок.Добавить(
		СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов(Символы.Таб, Уровень)
...
Процедура КонвертироватьОбщееСвойство(Приемник, Знач КомпонентыОбмена, Знач ИмяСвойства,
	...
	Иначе
		...
		Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
			...
			КонвертацияСвойстваXDTOВЭлементСтруктуры(ЗначениеXDTO, ПодСвойство, Приемник, КомпонентыОбмена, ,
...
Процедура КонвертироватьКлючевоеСвойство(Приемник, Знач КомпонентыОбмена, Знач ИмяСвойства,
	...
	Для Каждого КлючевоеСвойство Из ЗначениеXDTO.Свойства() Цикл
		...
		КонвертацияСвойстваXDTOВЭлементСтруктуры(ЗначениеXDTO, КлючевоеСвойство, Значение, КомпонентыОбмена, ,
			ОписаниеСвойства.Тип);
	КонецЦикла;
...
Процедура КонвертироватьСоставноеСвойство(Приемник, Знач КомпонентыОбмена, Знач ИмяСвойства,
	...
	Для Каждого ПодСвойство Из ЗначениеXDTO.Свойства() Цикл
		...
		КонвертацияСвойстваXDTOВЭлементСтруктуры(ЗначениеXDTO, ПодСвойство, Приемник, КомпонентыОбмена, ИмяСвойства,
			ОписаниеСвойства.Тип);
		Прервать;
...
//
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, Знач ПравилоКонвертации, ДанныеXDTO,
	...
	ЗаписыватьОбъектВИБ = Действие = "КонвертироватьИЗаписать";
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Попытка
		...
		ДанныеДляЗаписиВИБ = СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ЗначениеСвойства, ПравилоКонвертации,
			"ПолучитьСсылку");
	...
	Исключение
		ОбработатьИсключение(ИнформацияОбОшибке(),
			СтрШаблон(НСтр("ru = 'Ошибка конвертации свойства объекта XDTO, имя свойства: <%1>.'"),
				ПКС.СвойствоКонфигурации));
	КонецПопытки;
	
КонецПроцедуры

Функция СсылкаОбъектаПоСвойствамОбъектаXDTO(ПравилоКонвертации, ПолученныеДанные, ДанныеXDTOСодержатСсылку,
